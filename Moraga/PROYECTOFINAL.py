...
Densicomunal= numpy.zeros(ncomunas)
# Ciclo por comunas
...
#PRUEBA
	...
	plt.plot(psp[k][:,0],psp[k][:,1])	
        ...
        INTpspz[j] += INTzi
        AREApsp[j] += AREAi
    Densicomunal[j]+=H[j]/AREApsp[j]
    ICX=Densicomunal[j]*R*INTpspx[j]
    ICy=Densicomunal[j]*R*INTpspy[j]
    ICz=Densicomunal[j]*R*INTpspz[j]
    IX=IX+ICX
    IY=IY+ICy
    IZ=IZ+ICz
    POBLA=POBLA+H[j]
